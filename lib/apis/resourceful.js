...
function makeResource(_name, relPath) {
	...
	var newResource = function (entity, model) {
		var resource = _formatters[model || 'default'];
	...
	newResource.format = fluent(function (formatters) {
		if (isFunction(formatters)) {
			_formatters['default'] = formatters;
			return;
		}

		Object.keys(formatters).forEach(function (name) {
			if (!isFunction(formatters[name])) return;
			_formatters[name] = formatters[name];
		});
	});
