...
function makeResource(_name, relPath) {
	...
	var newResource = function (entity, model) {
		var resource = _models[model || 'default'];
	...
	newResource.model = fluent(function (models) {
		if (isFunction(models)) {
			_models['default'] = models;
			return;
		}

		Object.keys(models).forEach(function (name) {
			if (!isFunction(models[name])) return;
			_models[name] = models[name];
		});
	});
