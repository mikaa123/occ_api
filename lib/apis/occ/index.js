...
var express = require('express'),
	...
	makeGroup = require('catnap').makeGroup,
	extend = require('../../util/algebra').extend,
	fs = require('fs');

var api = makeGroup();

api.router = extend(express.Router(), {
	apiName: 'occ',
	root: '/api',
	doc: __dirname + '/occApi.apibp',
});
...
function answerMiddleware (req, res, next) {
	res.answer = function (params, code) {
		res.set('Content-Type', 'application/hal+json');
		res.send(code || 200, JSON.stringify(params, null, 2));
	};
...
api.router.use(answerMiddleware);
